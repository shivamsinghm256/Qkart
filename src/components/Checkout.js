...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    if(localStorage.getItem("balance")<getTotalCartValue(items)){
      enqueueSnackbar("/not have enough balance/i", {
        variant: "warning",
      });
      return false;
    }
    ...
    if(!addresses.selected.length){
      enqueueSnackbar("/select one shipping address/i", {
        variant: "warning",
      });
      return false;
    }
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      const newBalance = localStorage.getItem("balance") - getTotalCartValue(items);
